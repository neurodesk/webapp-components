...
| Optional Mindgrab | `apps/deface/src/mindgrab` and its conform/inverse-mapping integration | Add later after comparison on synthetic T1 images. The current browser implementation requires WebGPU `shader-f16`; it does not supply the desired WASM CPU default. Use a brain mask without Deface's optional 8 mm margin. |
