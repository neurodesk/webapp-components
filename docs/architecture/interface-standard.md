...
Keep scientific About content and connect its handler with `controlsContract` in `mountImagingWorkspace`, or `data-neurodesk-control="about|privacy|standalone"` in static HTML. The shared bar invokes that handler, hides its original trigger, and appends the shared "Under the hood" and "About this app" block (packages, builder, lightNIIng ecosystem statement and link) from `registry/app-information.yml`. Cite is rendered entirely by the shared bar from that registry: one entry per implemented method plus the Neurodesk platform paper. Do not write citation dialogs in apps; add the method to the registry and to `test/app-information.test.mjs`. Standalone is optional and appears only when an app registers it. Mark legacy catalog/source links with `data-neurodesk-shell-link` so the shell can hide them while standalone apps retain navigation. Links to particular methods or dependencies inside explanatory content are legitimate references.
...
Use the registered shell adapter and shared imaging layout when they fit the workflow. Build the sidebar, viewer chrome, technical log, status bar and information dialogs from the vocabulary in [the design system](design-system.md): `details.nd-sidebar-section`, `.nd-field`, `.nd-file`, `.nd-btn-primary`, `.nd-volume-toggle` result rows, `renderViewerToolbar`, `renderConsole` and `createInfoDialog`. Use shared design tokens for color, type, spacing, borders and focus treatment. Fix a repeated layout problem in its shared owner instead of adding app-ID CSS branches; `test/design-system.test.mjs` rejects app CSS that restyles the vocabulary or introduces colour literals.
...
New apps start from `pnpm new-app` and the canonical template, which is a complete input → task → output workspace built from the shared vocabulary. Keep the documentation template mirror synchronized (the design-system test compares them). Do not copy an entire app's styles as a starting point.
