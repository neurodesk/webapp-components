...
test('compact help, standalone commands and result switching remain reachable',async({page})=>{
 ...
 await expect(page.locator('.viewer-actions #opacity')).toHaveCount(1);
 await page.locator('#runButton').click();
 ...
 await expect(page.locator('#infoBody')).toContainText('NeuroImage, 54(3), 2033–2044');
 await expect(page.locator('#infoBody a[href="https://doi.org/10.1016/j.neuroimage.2021.118206"]')).toHaveCount(1);
});
