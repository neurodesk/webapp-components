...
import {templateAsset,browserSynthstrip} from '../../../packages/syncro/src/assets.js';
import './styles.css';
mountImagingWorkspace({controls:'#controls',viewer:'#viewer',status:'#status',title:'SYNcro',subtitle:'Brain normalization to MNI space',mark:'S',controlsContract:{about:'#aboutBtn',cite:'#citeBtn',privacy:'#privacyBtn'}});
const $=id=>document.getElementById(id),base=new URL(import.meta.env.BASE_URL,location.href);
let source,worker,viewer,viewerReady,busy=false,outputs,additional=[],timer,start,exampleAbort;
const status=m=>$('statusText').textContent=m;
const log=m=>{$('log').textContent=($('log').textContent+'\n'+m).slice(-18000);};
function setBusy(value){busy=value;for(const id of ['input','example','additional','modality','synthsrBackend','localSr','localStrip'])$(id).disabled=value;for(const s of $('additionalList').querySelectorAll('select'))s.disabled=value;$('runButton').disabled=value||!source;$('cancel').hidden=!value;$('download').disabled=value||!outputs;if(!value){clearInterval(timer);worker?.terminate();worker=null;}}
...
async function load(file){if(busy||!file)return;try{if(!/\.nii(\.gz)?$/i.test(file.name))throw new Error('Choose a NIfTI image (.nii or .nii.gz).');const v=readVolume(await file.arrayBuffer());source=file;outputs=null;$('results').open=false;$('resultSelect').disabled=true;$('download').disabled=true;$('originalView').disabled=false;$('fileInfo').textContent=file.name+' · '+v.dims.join(' × ');$('runButton').disabled=true;$('progress').value=0;await show(file);$('runButton').disabled=false;status('Ready to normalize · '+file.name);}catch(e){source=null;$('runButton').disabled=true;$('originalView').disabled=true;status(e.message);}}
$('input').onchange=()=>load($('input').files[0]);$('originalView').onclick=()=>show(source);
$('additional').onchange=()=>{additional=Array.from($('additional').files).map(file=>({file,type:'image'}));$('additionalList').replaceChildren();additional.forEach((item,i)=>{const label=document.createElement('label');label.htmlFor='type-'+i;label.textContent=item.file.name;const select=document.createElement('select');select.id='type-'+i;for(const [value,text]of [['image','Continuous image'],['binary','Binary lesion (0/1)'],['labels','Categorical labels']])select.add(new Option(text,value));select.onchange=()=>item.type=select.value;$('additionalList').append(label,select);});};
...
$('runButton').onclick=()=>{
 if(!source||busy)return;outputs=null;setBusy(true);status('Preparing normalization…');$('progress').value=0;$('results').open=false;$('resultSelect').disabled=true;$('log').textContent='';start=performance.now();timer=setInterval(()=>$('elapsed').textContent=Math.round((performance.now()-start)/1000)+' s',1000);
 ...
 worker.onmessage=({data})=>{
  ...
  if(data.type==='result'){outputs=data.outputs;setBusy(false);$('progress').value=1;status('Normalization complete · review the MNI alignment');$('results').open=true;$('resultSelect').replaceChildren();for(const name of Object.keys(outputs))if(/\.nii(\.gz)?$/.test(name)&&!name.includes('Warp'))$('resultSelect').add(new Option(name,name));$('resultSelect').value='warped-original.nii.gz';$('resultSelect').disabled=false;$('opacity').disabled=false;$('resultSelect').dispatchEvent(new Event('change'));}
 };
 worker.postMessage({input:source,additional,ct:$('modality').value==='ct',synthsrBackend:$('synthsrBackend').value,localSr:$('localSr').files[0],localStrip:$('localStrip').files[0],modelBase:import.meta.env.VITE_SYNCRO_MODEL_BASE,templateURL:templateAsset.url,registrationURL:new URL('registration/syncro-registration.mjs',base).href});
};
$('cancel').onclick=()=>{exampleAbort?.abort();setBusy(false);status('Processing cancelled');$('progress').value=0;};
$('resultSelect').onchange=()=>{const name=$('resultSelect').value;if(outputs?.[name])show(new File([outputs[name]],name),name.startsWith('warped-'));};
...
$('browserModelLink').href=browserSynthstrip.url;
$('packageLink').href=new URL('downloads/neurodesk-syncro-0.1.0.tgz',base).href;
...
window.addEventListener('pagehide',()=>{worker?.terminate();exampleAbort?.abort();clearInterval(timer);});
