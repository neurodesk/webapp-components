...
export async function runMindgrab({volume,assetPath,backend='auto',segmenter=segment,onProgress=()=>{},onLog=()=>{}}) {
  ...
  const result=await segmenter(writeVolume(volume,'MindGrab input'),{
    model:'mindgrab',mask:true,gzipOutput:false,backend,worker:false,assetPath,onLog,
  });
  if(!result.mask)throw new Error('MindGrab did not return a brain mask.');
  const brain=readVolume(asBuffer(result.image)),decodedMask=readVolume(asBuffer(result.mask));
  const mask={...decodedMask,data:Uint8Array.from(decodedMask.data,value=>value?1:0)};
  onProgress(1,'Brain extraction complete');
  return {brain,mask,provenance:{
    method:'MindGrab',package:'@brainchop/mindgrab',version:mindgrabPackage.version,
    model:'mindgrab',backend:result.backend,elapsedMs:result.elapsedMs,ranInWorker:result.ranInWorker,
  }};
}
