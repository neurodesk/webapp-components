...
async function modelBytes(model) {
  let cache;
  try { cache = await caches.open('neurodesk-synthseg-v1'); } catch { /* caching is optional */ }
  let response = await cache?.match(model.url);
  if (!response) {
    response = await fetch(model.url);
    if (!response.ok || response.headers.get('content-type')?.includes('text/html')) throw new Error('Could not download the SynthSeg weights. Check the connection and try again.');
  ...
  if (hash !== model.sha256) { await cache?.delete(model.url); throw new Error('Model checksum mismatch. The downloaded file was rejected.'); }
