...
const bin=resolve(process.argv[2]||'packages/syncro/bin/syncro.js');
const work=await mkdtemp(join(tmpdir(),'syncro-package-check-'));
function run(args){const r=spawnSync(process.execPath,[bin,...args],{encoding:'utf8',timeout:30000});if(r.error)throw r.error;return r;}
try {
 ...
 assert.equal(await readFile(join(output,'keep.txt'),'utf8'),'preserve');
 console.log('PASS installed package: help, output preservation, thread validation, offline cache failure, invalid resume');
