...
// discarded channel. Callers pad the channel count to a multiple of four first.
...
// `padded` zero-fills the output-channel axis so a classifier head with a channel
// count that is not a multiple of four still addresses one vec4 row per four outputs.
export function packConvWeights(values, [outputChannels,inputChannels,...kernel], padded = outputChannels) {
  const taps=kernel.reduce((a,b)=>a*b,1), packed=new Float32Array(taps*inputChannels*padded);
  for(let o=0;o<outputChannels;o++) for(let i=0;i<inputChannels;i++) for(let k=0;k<taps;k++) {
    packed[(k*inputChannels+i)*padded+o]=values[(o*inputChannels+i)*taps+k];
  }
  return packed;
}
